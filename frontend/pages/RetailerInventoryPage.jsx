        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200/60 flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
          <div className="flex gap-2 w-full md:w-auto p-1 bg-slate-50 rounded-xl">
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'my-stock' ? 'bg-white text-emerald-700 shadow-sm border border-slate-200/60' : 'text-slate-500 hover:text-slate-700'}`}
            >
            ...
            <button
              ...
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'all' ? 'bg-white text-emerald-700 shadow-sm border border-slate-200/60' : 'text-slate-500 hover:text-slate-700'}`}
            >
                      ...
                      {pagedInventory.map((med) => {
                        ...
                        return (
                          <tr key={med.id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                              <p className="text-sm font-extrabold text-[#15803d]">₹{(med.selling_price || med.mrp || 0).toFixed(2)}</p>
                              {med.mrp && med.selling_price && med.mrp !== med.selling_price && <p className="text-[10px] font-black text-slate-400 line-through">₹{med.mrp.toFixed(2)}</p>}
                            </td>
                            <td className="px-6 py-4">
                              {isOutOfStock ? (
                    ...
                    <div key={med.id} className="bg-white rounded-[1.5rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group relative flex flex-col border border-slate-100">
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        ...
                        <div className="mt-auto flex items-end justify-between pt-2 border-t border-slate-50">
                          <div>
                            {med.mrp && med.selling_price && med.mrp !== med.selling_price && <span className="text-[10px] font-bold text-slate-400 line-through block mb-0.5">₹{med.mrp.toFixed(2)}</span>}
                            <span className="text-xl font-extrabold text-[#15803d] tracking-tight">₹{(med.selling_price || med.mrp || 0).toFixed(2)}</span>
                          </div>
                ...
                <div className="flex items-center gap-4 mb-6 p-4 bg-[#f8f9fa] rounded-2xl border border-slate-100">
                  ...
                  <div className="flex-1 pl-2">
                    ...
                    <div className="flex items-center gap-2">
                       <span className={`text-sm font-black ${ stockModal.mrp !== stockModal.selling_price ? 'text-slate-400 line-through' : 'text-slate-900' }`}>₹{stockModal.mrp?.toFixed(2) || '0.00'}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="md:w-7/12 p-6 md:p-10 bg-white flex flex-col justify-center overflow-y-auto">
              <div>
                ...
                <p className="text-sm font-medium text-slate-500 mb-8 leading-relaxed">
                  {modalMode === 'add' ? 'Add this medicine to your retail inventory. Set initial stock levels and reorder thresholds.' : 
                   modalMode === 'update' ? 'Adjust current local stock manually and tweak reorder alert points.' :
                ...
                {(modalMode === 'add' || modalMode === 'update') && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[14px]">notifications_active</span> Alert At
                      </label>
                      <div className="relative">
                        <input
                        ...
                        />
                        <span className="absolute right-5 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400 pointer-events-none uppercase tracking-wider">units</span>
                      </div>
                    </div>
                  </div>
                )}
