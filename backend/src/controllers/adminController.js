...
exports.getAnalytics = async (req, res) => {
  try {
    ...
    const monthly = await sequelize.query(
      `SELECT EXTRACT(MONTH FROM "createdAt")::int as month, COUNT(*) as orders, COALESCE(SUM(total),0) as revenue
       FROM orders WHERE "createdAt" >= NOW() - INTERVAL '12 months'
    ...
    res.json({ dailyOrders, monthly, statusBreakdown });
  } catch (err) { res.status(500).json({ message: err.message }); }
};
