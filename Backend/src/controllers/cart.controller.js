...
exports.addToCart = async (req, res) => {
  try {
    const userId = req.user.userId;
    const { medicineId, productId, quantity = 1 } = req.body;
    const itemId = medicineId || productId;

    if (!itemId) {
      return res.status(400).json({ error: 'medicineId or productId is required' });
    }
    ...
    if (cartRes.rows.length === 0) {
      try {
        ...
        // If concurrent requests try to create the cart at the exact same time, catch the unique violation
        if (insertErr.code === '23505') {
          const retryRes = await db.query('SELECT id FROM carts WHERE user_id = $1 AND is_active = true LIMIT 1', [userId]);
          cartId = retryRes.rows[0].id;
        } else {
          throw insertErr;
        }
      }
    ...
    // Get medicine price
    const medRes = await db.query('SELECT selling_price FROM medicines WHERE id = $1', [itemId]);
    ...
    const existingItem = await db.query('SELECT id, quantity FROM cart_items WHERE cart_id = $1 AND medicine_id = $2', [cartId, itemId]);
    ...
    if (existingItem.rows.length > 0) {
      ...
      await db.query(
        ...
        [cartId, itemId, quantity, unitPrice, totalPrice]
      );
    }
