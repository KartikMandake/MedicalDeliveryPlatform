...
## API Layer (Cart + Tracking)

This project now includes a backend API in `server/app.js` for cart and order tracking workflows.

### API Prerequisites

- PostgreSQL with PostGIS enabled
- Run schema + seed from the `db/` folder
- Add environment variables by creating `.env` from `.env.example`
...
- `POST /cart/items`
   - Adds or increments medicine quantity in user's active cart.
   - Body: `{ "userId": "<uuid>", "medicineId": "<uuid>", "quantity": 1 }`

- `GET /cart/:userId`
   - Returns active cart items and summary for a user.

- `PATCH /cart/items/:itemId`
   - Updates quantity (set `0` to remove item).
   - Body: `{ "quantity": 2 }`

- `POST /agent/location`
   - Upserts live location from agent mobile app.
   - Body: `{ "agentId": "<uuid>", "lat": 12.97, "lng": 77.60, "isOnline": true }`

- `GET /orders/:orderNumber/tracking`
   - Returns tracking details with dynamic tracking mode:
      - `agent_live_location` for `picked_up` and `in_transit`
      - `retailer_stage` for `confirmed`, `packing`, `ready`

- `PATCH /orders/:id/status`
   - Updates order status (`placed`, `confirmed`, `packing`, `ready`, `picked_up`, `in_transit`, `delivered`, `cancelled`).

### Frontend integration now wired

- Products page add button calls `POST /cart/items`.
- Cart page loads from `GET /cart/:userId` and quantity controls call `PATCH /cart/items/:itemId`.
- Tracking page polls `GET /orders/:orderNumber/tracking` every 7 seconds.
- You can pass order number via URL query: `/tracking?orderNumber=ORD-1001`.
